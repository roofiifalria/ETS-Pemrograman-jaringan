...
def process_client_request_mp(connection_fd, address_tuple):
    """
    Fungsi ini akan dijalankan oleh setiap proses di ProcessPoolExecutor.
    Menerima file descriptor socket dan alamat klien sebagai tuple.
    """
    conn = socket.socket(socket.AF_INET, socket.SOCK_STREAM, fileno=connection_fd)
    ...
    try:
        conn.settimeout(120) # Set timeout ke 120 detik (2 menit)
        ...
        if not header_str:
            logging.info(f"No command header received from {client_address}. Closing connection.")
            return
                ...
                try:
                    ...
                    if hasil_dict.get('status') == 'OK':
                        try:
                            ...
                            logging.info(f"UPLOAD of {filename_to_upload} from {client_address} completed. Original size: {total_uploaded_bytes} bytes.")
                    ...
                    logging.error(f"Failed to decode UPLOAD body as UTF-8 for {client_address} for {filename_to_upload}. This might indicate corrupted Base64 data.", exc_info=True)
                    hasil_dict = dict(status='ERROR', data="Failed to decode file content.")
                except Exception as e:
                    logging.error(f"Error processing UPLOAD for {filename_to_upload} from {client_address}: {e}", exc_info=True)
        ...
        logging.error(f"Socket timeout for {client_address} during receive. Closing connection.", exc_info=True)
        hasil_dict = dict(status='ERROR', data="Server receive timeout.")
    except Exception as e:
        logging.error(f"Error processing client {client_address} command: {e}", exc_info=True)
        ...
        try:
            ...
            logging.error(f"Error sending response to {client_address}: {e}", exc_info=True)
        finally:
            conn.close()

class ServerProcessPool(multiprocessing.Process):    
    ...
    def run(self):
        ...
        logging.info(f"Server (ProcessPool) running on {self.ipinfo} with {self.executor._max_workers} workers")
        try:
            ...
            while not self._shutdown_event.is_set():
                try:
                    ...
                    connection.close() # Tutup socket di proses induk segera

                    if not self._shutdown_event.is_set():
                        self.executor.submit(process_client_request_mp, conn_fd, client_address)
                    else:
...
def main():
    ...
    server = ServerProcessPool(ipaddress='0.0.0.0', port=port, num_workers=num_workers_server)
    server.start()
